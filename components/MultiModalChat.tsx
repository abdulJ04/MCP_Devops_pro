...
interface MultiModalChatProps {
  // ✅ FIXED: returns the workflow id (string), so caller can track it
  onWorkflowCreate?: (name: string, status: 'success' | 'running' | 'failed', branch: string) => string | void;
  // ✅ NEW: lets the chat update an EXISTING workflow by id, instead of creating a new one
  onWorkflowUpdate?: (workflowId: string, status: 'success' | 'running' | 'failed') => void;
  agentType?: string;
}

export default function MultiModalChat({ onWorkflowCreate, onWorkflowUpdate, agentType = 'devops' }: MultiModalChatProps) {
  ...
  useEffect(() => {
    if (typeof window !== 'undefined') {
      ...
      const win = window as Window & typeof globalThis & { SpeechRecognition?: typeof SpeechRecognition; webkitSpeechRecognition?: typeof SpeechRecognition };
  ...
  const generateContextualResponse = (userInput: string): string => {
    ...
    if (lowerInput.includes('clone') && (lowerInput.includes('github') || lowerInput.includes('repo') || lowerInput.includes('repository'))) {
      const urlMatch = userInput.match(/https?:\/\/github\.com\/[^\s]+/);
      const repoUrl = urlMatch ? urlMatch[0] : 'https://github.com/example/random-repo';
      const repoName = repoUrl.split('/').pop()?.replace('.git', '') || 'random-repo';
      
      if (onWorkflowCreate) {
        // Create a workflow entry for tracking
        const workflowId = onWorkflowCreate(`Clone ${repoName}`, 'running', 'main') as string;
        
        // REAL MCP INTEGRATION: Call our MCP API proxy to perform the clone
        fetch('/api/v1/mcp', {
          method: 'POST',
          headers: { 'Content-Type': 'application/json' },
          body: JSON.stringify({ action: 'clone', repoUrl })
        })
          .then(res => res.json())
          .then(result => {
            if (onWorkflowUpdate && workflowId) {
              // Update the workflow to success or failed based on the result
              onWorkflowUpdate(workflowId, result.success ? 'success' : 'failed');
            }
          })
          .catch((err) => {
            console.error('Clone failed:', err);
            if (onWorkflowUpdate && workflowId) {
              onWorkflowUpdate(workflowId, 'failed');
            }
          });
      }
      
      return `🔄 Starting clone operation for ${repoUrl}...\n\n✅ Workflow created: "Clone ${repoName}"\n📊 Status: Running\n🌿 Branch: main\n\nI've added this to your Total Workflows and it's now showing in Running Workflows. The clone operation is in progress. You can monitor the update in the GitHub Workflow Status section above.\n\nWould you like me to perform any other operations?`;
    }
      
    // Handle MCP server commands
    if (lowerInput.includes('start') && lowerInput.includes('mcp')) {
      if (lowerInput.includes('dev') || lowerInput.includes('development')) {
        // Call MCP API to start development server
        fetch('/api/v1/mcp', {
          method: 'POST',
          headers: { 'Content-Type': 'application/json' },
          body: JSON.stringify({ server: 'development', action: 'start' })
        }).then(res => res.json()).then(result => {
          if (result.success && onWorkflowCreate) {
            const wfId = onWorkflowCreate('Start Development MCP', 'running', 'main');
            setTimeout(() => {
              if (onWorkflowUpdate && wfId) onWorkflowUpdate(wfId, 'success');
            }, 1000);
          }
        }).catch(console.error);
        
        return '🖥️ Starting the Development MCP server...\n\n• Endpoint: /api/v1/development\n\nI\'ve sent the start request. The server will be available shortly. Check the MCP Server Controls section for status updates.';
      } else if (lowerInput.includes('staging')) {
        fetch('/api/v1/mcp', {
          method: 'POST',
          headers: { 'Content-Type': 'application/json' },
          body: JSON.stringify({ server: 'staging', action: 'start' })
        }).then(res => res.json()).catch(console.error);
        return '🖥️ Starting the Staging MCP server...\n\n• Endpoint: /api/v1/staging\n\nCheck the MCP Server Controls section for status updates.';
      } else if (lowerInput.includes('production') || lowerInput.includes('prod')) {
        return '🖥️ The Production MCP server is already running. Check the MCP Servers section for status.';
      }
      return '🖥️ Which MCP server would you like to start? Options:\n\n• Development MCP\n• Staging MCP\n• Production MCP\n\nPlease specify which one (e.g., "start development mcp").';
    }
